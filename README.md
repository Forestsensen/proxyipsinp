# Cloudflare IP Auto Scanner & Sync (CF 优选 IP 自动同步器)

这是一个运行在 GitHub Actions 上的全自动 Cloudflare 优选 IP 扫描与 DNS 同步工具。它会自动生成大量 CF IP 并通过 API 测速，挑选出延迟最低的优选 IP（默认只挑选 **10 个 USA 节点**），并自动更新到你的 Cloudflare DNS 记录中。
...
CF_CIDRS = [
    ...
    # ==========================================
```
如果你在网上找到了某些玄学好用的 CF 优选段，直接把网段加到这个列表里，程序就会自动去里面随机抽卡测速了。
