...
# ==========================================
```
- **精准模式**：填入你想要的机场码（如 `SJC,LAX,HKG`），系统会一次性海扫，并自动把最快的节点分配到各自的子域名（如 `lax.你的主域名`、`sjc.你的主域名`）。
- **盲扫模式 (ALL)**：不知道填什么？直接填 `"ALL"`！系统会在全网疯狂盲扫，自动识别国家地区，并为你收割最多 20 个不同国家的最快极品节点。
- **免配置测速模式**：如果你只是想体验极致的测速快感，或者没有 Cloudflare 域名。没关系！**不配置环境变量也可以直接运行！** 程序会跳过 DNS 同步，但依然会将扫出的全部极品节点打印在屏幕上，并自动保存进 `ips-v4.txt` 中供你直接享用！
